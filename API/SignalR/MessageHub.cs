...
using API.Interfaces; // Interfaces for repositories like IMessageRepository and IUserRepository.
    ...
    public class MessageHub(IMessageRepository messageRepository, IUserRepository userRepository, IMapper mapper, IHubContext<PresenceHub> presenceHub) : Hub
    {
        ...
        private async Task<Group> AddToGroup(string groupName)
        {
            ...
            if (group == null)
            {
                ...
                messageRepository.AddGroup(group); // Add the group to the repository.
            }
        ...
        {
            var group = await messageRepository.GetGroupForConnection(Context.ConnectionId); // Get the group for the connection.
            var connection = group?.Connections.FirstOrDefault(x => x.ConnectionId == Context.ConnectionId); // Find the connection.

            if (connection != null && group != null)
            {
                messageRepository.RemoveConnection(connection); // Remove the connection from the repository.
        ...
        public async Task SendMessage(CreateMessageDto createMessageDto)
        {
            ...
            messageRepository.AddMessage(message); // Add the message to the repository.
